...
describe 'ReportJob: Delete', type: :request do
  ...
  subject(:action) { api.rel(:report_job).delete(id: report_job.id).value! }

  it 'responds with 204 No Content' do
    expect(subject.response.status).to eq :no_content
  end
  ...
  context 'for running report job' do
    ...
    it 'responds with 409 Conflict' do
      expect { subject }.to raise_error(Restify::ClientError) do |error|
