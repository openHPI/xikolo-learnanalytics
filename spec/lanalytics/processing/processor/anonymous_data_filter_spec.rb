...
describe Lanalytics::Filter::AnonymousDataFilter do
  ...
  let(:original_hash) { FactoryGirl.attributes_for(:amqp_user).with_indifferent_access }
  let(:data_filter) { Lanalytics::Filter::AnonymousDataFilter.new }
  let(:processed_resources) do
    [Lanalytics::Model::StmtResource.new(original_hash[:id], :USER, {
      language: "en",
  ...
  it 'should remove all sensitive data properties' do
    data_filter.filter(original_hash, processed_resources, opts = nil)
