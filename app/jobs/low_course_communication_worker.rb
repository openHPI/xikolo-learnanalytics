  ...
  def perform(course, rule_id)
    ...
    if course_is_active(course)
      ...
      if start_date < 10.days.ago and total_enrollments.to_i > enrollments_threshold
        current_announcements = Xikolo::News::News.where(course_id: course.id, published: "true", page: 1, per_page:1) #since its ordered desc
        Acfs.run
        if not current_announcements.first or current_announcements.first.publish_at <= 10.days.ago
          if current_announcements.first
            annnotation = 'Last announcement ' + current_announcements.first.publish_at.to_formatted_s(:short)
          end
