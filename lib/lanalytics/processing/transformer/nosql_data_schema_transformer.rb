Processing = Lanalytics::Processing
LoadORM    = Processing::LoadORM
    ...
    module Transformer
      ...
      class NosqlDataSchemaTransformer < TransformStep
        ...
        def transform(original_event, processing_units, load_commands, pipeline_ctx)
          if pipeline_ctx.processing_action == Processing::Action::CREATE
            transform_to_load_commands_for_create(processing_units, load_commands)
          elsif pipeline_ctx.processing_action == Processing::Action::UPDATE
            transform_to_load_commands_for_update(processing_units, load_commands)
          elsif pipeline_ctx.processing_action == Processing::Action::DESTROY
        ...
        def transform_to_load_commands_for_create(processing_units, load_commands)
          processing_units.each do |processing_unit|
            if processing_unit.type == :exp_event
              load_commands << LoadORM::CreateCommand.with(
                transform_exp_event_unit(processing_unit)
              )
              next
            elsif processing_unit.type == :question
              entities = transform_question_unit(processing_unit)
              load_commands << LoadORM::MergeEntityCommand.with(entities[0])
              load_commands << LoadORM::CreateCommand.with(entities[1])
              load_commands << LoadORM::CreateCommand.with(entities[2])
              next
            end
            ...
            if entities.is_a?(Array)
              load_commands.push(
                *entities.map do |entity|
                  LoadORM::MergeEntityCommand.with(entity)
                end
              )
            else
              load_commands << LoadORM::MergeEntityCommand.with(entities)
            end
          end
        end
        ...
        def transform_to_load_commands_for_update(processing_units, load_commands)
          processing_units.each do |processing_unit|
            ...
            if entities.is_a?(Array)
              load_commands.push(
                *entities.map do |entity|
                  LoadORM::UpdateCommand.with(entity)
                end
              )
            else
              load_commands << LoadORM::UpdateCommand.with(entities)
            end
          end
        end
        ...
        def transform_to_load_commands_for_destroy(processing_units, load_commands)
          processing_units.each do |processing_unit|
            ...
            if processing_unit.type == :enrollment
              relationship = handle_directed_relationship(
                processing_unit,
                :UN_ENROLLED,
                :user_id,
                :course_id
              )
              load_commands << LoadORM::MergeEntityCommand.with(relationship)
              next
            ...
            if entities.is_a?(Array)
              load_commands.push(*entities.map { |entity| LoadORM::DestroyCommand.with(entity) })
            else
              load_commands << LoadORM::DestroyCommand.with(entities)
            end
          end
        end
        ...
        def transform_item_unit(processing_unit)
          item_entity = new_entity_template(
            processing_unit,
            [
              :title,
              :content_type,
          ...
          item_course_rel = LoadORM::EntityRelationship.create(:BELONGS_TO) do
        ...
        def transform_learning_room_unit(processing_unit)
          ...
          learning_room_course_rel = LoadORM::EntityRelationship.create(:BELONGS_TO) do
          ...
          [learning_room_entity, learning_room_course_rel]
        end
        ...
        def transform_question_unit(processing_unit)
          question_entity = new_entity_template(
            processing_unit,
            [
              :title,
              :text,
          ...
          user_question_rel = LoadORM::EntityRelationship.create(:POSTED) do
          ...
          if processing_unit[:course_id]
            question_course_rel = LoadORM::EntityRelationship.create(:BELONGS_TO) do
            ...
            question_learning_room_rel = LoadORM::EntityRelationship.create(:BELONGS_TO) do
              with_from_entity(:QUESTION) do
                with_primary_attribute :resource_uuid, :uuid, processing_unit[:id]
              end
        ...
        def transform_answer_unit(processing_unit)
          ...
          user_answer_rel = LoadORM::EntityRelationship.create(:POSTED_ANSWER) do
          ...
          answer_question_rel = LoadORM::EntityRelationship.create(:BELONGS_TO) do
          ...
          [answer_entity, user_answer_rel, answer_question_rel]
        end

        def transform_comment_unit(processing_unit)
          LoadORM::EntityRelationship.create(:COMMENTED) do
        ...
        def transform_ticket_unit(processing_unit)
          helpdesk_ticket_entity = new_entity_template(
            processing_unit,
            [:url, :language, :mail, :report, :title, :data, :created_at]
          )

          result = [helpdesk_ticket_entity]

          if processing_unit[:user_id]
            result << LoadORM::EntityRelationship.create(:SUBMITTED_FEEDBACK_FROM) do
              with_from_entity(:USER) do
                with_primary_attribute :resource_uuid, :uuid, processing_unit[:user_id]
              end
          ...
          if processing_unit[:course_id]
            result << LoadORM::EntityRelationship.create(:SUBMITTED_FEEDBACK_FOR) do
