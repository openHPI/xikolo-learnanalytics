  ...
  module Metric
    class Backjumps < GoogleAnalyticsMetric
      ...
      exec do |params|
        ...
        items = params[:item_id].present? ? all_items.select{ |item| item.id == params[:item_id] } : all_items
        ...
        result = request_reports(items.map do |item|
        ...
        # Process results
        processed_result = {}
        processed_result[:items] = items.map do |item|
