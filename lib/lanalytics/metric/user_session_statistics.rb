  ...
  module Metric
    class UserSessionStatistics < Base
      ...
      def self.datasource_keys
        %w(exp_api_elastic exp_api_native google_analytics)
      end
      ...
      exec do |params|
        ...
        elastic = Lanalytics::Processing::DatasourceManager.datasource('exp_api_elastic')
        ...
        user_count = user_result['aggregations']['distinct_user_count']['value'].to_f
...

        {
          ...
          course_total_sessions: session_durations[:total_session_duration] / user_count,
          course_sessions: session_durations[:total_sessions] / user_count
        }
      end
    end
  end
